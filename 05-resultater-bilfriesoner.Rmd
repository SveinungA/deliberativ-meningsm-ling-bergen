...
Det er høy oppslutning om å redusere farten til 30 km/t i svært tettbygde strøk. `r round(sum((dp_t_post$fill=='Mindretall') / (sum(dp_t_post$fill=='Mindretall')+(sum(dp_t_post$fill=='Flertall'))))*100, 0)` prosent støtter  dette forslaget, og av disse igjen er overvekten på svarkategorien som indikerer ekstremt sterk støtte.
`r round(sum((dp_t_post$fill=='Flertall') / (sum(dp_t_post$fill=='Mindretall')+(sum(dp_t_post$fill=='Flertall'))))*100, 0)` prosent motsetter seg forslaget.
