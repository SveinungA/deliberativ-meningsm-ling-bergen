...
### Redusere farten i svært tettbygde strøk.
...
Det er svært høy oppslutning om å redusere farten til 30 km/t i svært tettbygde strøk. `r round(sum((post_t_delib$fill=='Flertall') / (sum(post_t_delib$fill=='Mindretall')+(sum(post_t_delib$fill=='Flertall'))))*100, 0)` prosent støtter  dette forslaget, og av disse igjen er overvekten på svarkategorien som indikerer ekstremt sterk støtte.
`r round(sum((post_t_delib$fill=='Mindretall') / (sum(post_t_delib$fill=='Mindretall')+(sum(post_t_delib$fill=='Flertall'))))*100, 0)` prosent motsetter seg forslaget.
