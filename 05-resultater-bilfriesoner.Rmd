...
```{r car-results-manage-1, include=FALSE, eval=FALSE}
...
Q16_c <- dp_c %>% group_by(survey) %>% summarise(mean=mean(Q16, na.rm=TRUE), se=sd(Q16, na.rm = TRUE)/sqrt(length(Q16))) %>% mutate(group = 'treatment', question = 'Q16')
...
Q16_t <- dp_t %>% group_by(survey) %>% summarise(mean=mean(Q16, na.rm=TRUE), se=sd(Q16, na.rm = TRUE)/sqrt(length(Q16))) %>% mutate(group = 'treatment', question = 'Q16')
...
car <- bind_rows(list(Q11_c, Q12_c, Q13_c, Q14_c, Q15_c, Q16_c, Q17_1_c, Q17_2_c, Q17_3_c, Q17_4_c, Q17_5_c, Q17_6_c, Q18_c, Q19_c, Q20_c, Q21_c, Q11_t, Q12_t, Q13_t, Q14_t, Q15_t,  Q16_t, Q17_1_t, Q17_2_t, Q17_3_t, Q17_4_t, Q17_5_t, Q17_6_t, Q18_t, Q19_t, Q20_t, Q21_t), .id = "group") %>% 
 mutate(group = (case_when(group %in% c(1:16) ~ 'Kontrollgruppe',
                                               group %in% c(17:32) ~ 'Deltakergruppe')),
                            question = (case_when(question == 'Q11' ~ 'Bergen sentrum mer bilfritt',
                                                  ...
                                                  question == 'Q15' ~ 'Andel parkeringsplasser til private biler vs. bildeling',
                                                   question == 'Q16' ~ 'Andel parkeringsplasser til private biler vs. bildeling',
...
```{r car-results-manage-2, include=FALSE, eval=FALSE}
...
```{r car-results-figure2, include=FALSE, eval=FALSE}
...
within <- full_join(within_post, within_pre, by = 'Q1', keep=TRUE) %>%
  filter(Q1.x == Q1.y) 
within <-   within[-c(8, 50), ] #fjern duplikater
...
within_summary <- within %>% summarise(smean = across(.cols=everything(), ~ round(mean(.x, na.rm = TRUE), digits=2)),
                                       sd = across(.cols=everything(), ~ round(sd(.x, na.rm = TRUE), digits = 2)),
                                       count = across(.cols=everything(), ~ round(n(), digits = 0)))
#Trenger standardfeil og konfidensintervaller. Hvordan?
                                         
within_summary <- pivot_longer(within_summary, cols = everything(), names_to = "stats")

#Gjenstår: 1. endring for kontrollgruppen og å lage tabell
```
