...
p4 <- ggplot(inntekt, aes(x = factor(Q30))) +
  geom_bar(aes(y = (..count..) / sum(..count..))) +
   scale_y_continuous(labels = scales::percent_format(accuracy = 1L), breaks = seq(0,1, by = 0.2)) +
  scale_x_discrete(labels = c('<150K', '150K-300K', '300K-400K', '400K-500K', '500K-600K', '600K-700k', '700K-800K', '800K-900K', '900K-1M', '>1M'), guide = guide_axis(n.dodge = 3)) +
...
Kriteriet for å bli del av analysene er at man har deltatt i deliberasjonen og svart på etterundersøkelsen.
...
Vi noterer oss at deltakelsen er høy i de yngre aldersgruppene, og lavere blant de eldre.
...
Slik sett hadde det vært mer ønskelig med en aldersmessig jevnere deltakelse.
Når det er sagt, er det ofte de unge som er underrepresentert i andre demokratiske kanaler (som valg), så det er verdt å merke seg at dette arrangementet nådde fram til denne gruppen.
Et fysisk arrangement ville senket den digitale kunnskapsterskelen, men samtidig introdusert andre terskler som for eksempel kunne gått ut over deltakelsen til personer som har vanskelig for å møte opp i et lokale av praktiske grunner.

Geografisk manglet det deltakere fra Arna.
